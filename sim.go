...
func (sim *ChandyLamportSim) CollectSnapshot(snapshotId int) *GlobalSnapshot {
	...
	// TODO: Complete this method
	done := make(chan bool, len(sim.snapstaken[snapshotId]))
	for _, v := range sim.snapstaken[snapshotId] {
		go func(done chan bool, v chan bool) {
			<-v
			done <- true
		}(done, v)
	}
	for range sim.snapstaken[snapshotId] {
		<-done
	}
	...
	for _, nodeId := range getSortedKeys(sim.nodes) {
		server := sim.nodes[nodeId]
		server.ssstate.Range(func(k, v interface{}) bool {
			if key, isInt := k.(int); isInt && key == snapshotId {
				if snapshot, ok := v.(*Snapshot); ok {
					tokens[nodeId] = snapshot.tokens
	...
	snap := GlobalSnapshot{snapshotId, tokens, messages}

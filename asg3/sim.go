...
type ChandyLamportSim struct {
	...
	snapstaken map[int]map[string]chan bool
}
...
func NewSimulator() *ChandyLamportSim {
	return &ChandyLamportSim{
		...
		snapstaken: make(map[int]map[string]chan bool),
	}
}
...
func (sim *ChandyLamportSim) StartSnapshot(nodeId string) {
	...
	sim.snapstaken[snapshotId] = make(map[string]chan bool)
	for nodeId, _ := range sim.nodes {
		sim.snapstaken[snapshotId][nodeId] = make(chan bool)
	}
	server, err := sim.nodes[nodeId]
	if err {
		panic(err)
	}
...
func (sim *ChandyLamportSim) NotifyCompletedSnapshot(nodeId string, snapshotId int) {
	...
	// TODO: Complete this method
	if node := sim.nodes[nodeId]; node != nil {
		sim.snapstaken[snapshotId][nodeId] <- true
	}
...
func (sim *ChandyLamportSim) CollectSnapshot(snapshotId int) *GlobalSnapshot {
	...
	wg.Add(len(sim.snapstaken[snapshotId])) // for blocking until all snapshots of nodes are collected
	for _, nodeSnap := range sim.snapstaken[snapshotId] {
		go func(nodeSnap chan bool) {
			defer wg.Done()
			<-nodeSnap // for blocking until each node responds
		}(nodeSnap)
	}
	wg.Wait()
	...
	for _, serverId := range getSortedKeys(sim.nodes) {
		server := sim.nodes[serverId]
